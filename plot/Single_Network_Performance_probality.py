...
if __name__ == '__main__':
    ...
    r = 50/100
    #Vehicle_N = [3]
    for comp_source in Comp_source:
        comp_id = comp_source
        Latency_comp[comp_source] = []
        comp_source = comp_source * GHZ
        Latency_network = {"mmwave": [], "DSRC": [], "C-V2I": []}
        for violation_probability in range(1, 100000, 2000):
            violation_probability /= 100000
            for v2x in V2X:
                propotion = np.zeros(len(V2X))
                propotion[v2x_id[v2x]] = 1
                #propotion = np.ones(len(V2X)) * (1 / 3)
        ...
        Latency_comp[comp_id].append(Latency_network)
    ...
    for i in range(len(Comp_source)):
        comp_id = Comp_source[i]
        for j in range(len(V2X)):
            Task_dict =   Latency_comp[comp_id][0]
            v2x = V2X[j]
            vehicle_d = Task_dict[v2x]
            MakerSC = MakerShape[j]+MakerColor[i]+LineShape[i]
            plt.plot(vehicle_d,y_r,MakerSC,markevery=100)


    font2 = {'family': 'Times New Roman',
             'weight': 'normal',
             'size': 10} # 字体大小，相当于也是leged框的大小
    plt.legend(["mmwave:"+r"$f_{E}$"+"=50000 Gcycles/s","DSRC:"+r"$f_{E}$"+"=50000 Gcycles/s","C-V2I:"+r"$f_{E}$"+"=50000 Gcycles/s",
                "mmwave:"+r"$f_{E}$"+"=100000 Gcycles/s","DSRC:"+r"$f_{E}$"+"=100000 Gcycles/s","C-V2I:"+r"$f_{E}$"+"=100000 Gcycles/s",
                "mmwave:"+r"$f_{E}$"+"=80000 Gcycles/s","DSRC:"+r"$f_{E}$"+"=80000 Gcycles/s","C-V2I:"+r"$f_{E}$"+"=80000 Gcycles/s"],
                 loc='upper left', bbox_to_anchor=(0.1, 0.6),fancybox=True, shadow=False, prop=font2) # Shadow：legend背景是否透明
    #plt.plot(x_r,Latency["mmwave"],'cx:',x_r,Latency["DSRC"],"rv--",x_r,Latency["C-V2I"],"*g-.",markevery=8)
    plt.yscale("log")
    plt.xscale("log")

    plt.ylabel("Violation Probability  " + r'$\epsilon^{w}$')
    plt.xlabel("Delay " + r'$\omega$')
