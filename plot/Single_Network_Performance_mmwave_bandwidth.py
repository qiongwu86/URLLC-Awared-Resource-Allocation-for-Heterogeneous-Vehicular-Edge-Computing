...
def net_mode(v2x,mmwave_b,gamm):
    if v2x == "DSRC":
        R_dsrc = dsrc_r*MBPS
        t_serv = 0.01
...
def NetServe(v2x,v2xid,R,B,mmwave_b,gamm=None):
    R = R.copy()
    B = B.copy()
    R[TaskId,v2xid] = 0
    B[TaskId,v2xid] = 0    # 这里的仿真是全部由一个通信来传输，所以没有竞争
    if v2x == "DSRC":
        Netmode, latency = net_mode(v2x, mmwave_b=None,gamm=None)
        xi_comm = max(Netmode - sum(R[:, v2x_id[v2x]]),0)
        eta_comm = sum(B[:,v2x_id[v2x]]) + latency # 加一个延迟
        return xi_comm, eta_comm
    elif v2x =="C-V2I":
        xi_comm = net_mode(v2x,mmwave_b=None,gamm=None)
        eta_comm = 0
        return xi_comm,eta_comm
    else:
        Netmode = net_mode(v2x, mmwave_b, gamm)
        xi_comm = max(Netmode - sum(R[:, v2x_id[v2x]]),0)
        eta_comm = sum(B[:,v2x_id[v2x]])
        return xi_comm,eta_comm

def CompServe(comp_source,vehicle_n,TaskId,v2xid,R,B,workload):
    ...
    print(comp_source / workload /vehicle_n,sum(R[TaskId,:]))
    eta_comp = sum(B[TaskId,:])
...
if __name__ == '__main__':
    ...
    #Vehicle_N = [5,10,20]
    vehicle = 5
    MMWAVE_b = [10,20,30,40,50,100,200]
    for mmwave_b in MMWAVE_b:
        Latency_comm[mmwave_b] = []
        ...
        for r in range(1,40,1):
            ...
            for v2x in V2X:
                ...
                xi_comp,eta_comp = CompServe(comp_source,vehicle,TaskId,v2x_id[v2x],R*propotion, B*propotion, workload)
                xi_comm,eta_comm = NetServe(v2x, v2x_id[v2x],R*propotion,B*propotion,mmwave_b,gamm=gamm)
        ...
        Latency_comm[mmwave_b].append(Latency_network)


    x_r = [i/10 for i in range(1,40,1)]
    MakerColor = ["r","g","b","m","b","g","r"]
    MakerShape = ["*","^","s","o","*","^","s"]
    LineShape = ["-","-.","-","-","-.","-","-."]
    MarkEvery = 20
    for i in range(len(MMWAVE_b)):
        for j in range(len(V2X)):
            n = MMWAVE_b[i]
            vehicle_n =  Latency_comm[n][0]
            v2x = V2X[j]
    ...
    MMWAVE_b = [10, 20, 30, 40, 50, 100, 200]
    plt.legend(["mmwave: 10 ","DSRC: 10 ","C-V2I: 10 ",
                "mmwave: 20 ","DSRC: 20 ","C-V2I: 20 ",
                "mmwave: 30 ","DSRC: 30 ","C-V2I: 30 ",
                "mmwave: 40 ","DSRC: 40 ","C-V2I: 40 ",
                "mmwave: 50 ","DSRC: 50 ","C-V2I: 50 ",
                "mmwave: 100 ","DSRC: 100 ","C-V2I: 100 ",
                "mmwave: 200 ","DSRC: 200 ","C-V2I: 200 ",],
