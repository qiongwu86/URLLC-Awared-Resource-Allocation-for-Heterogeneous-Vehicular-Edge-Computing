...
if __name__ == '__main__':
    #Comp_source = [500*GHZ,1000*GHZ,1500*GHZ,2000*GHZ,5000*GHZ,10000*GHZ,20000*GHZ]
    Comp_source = [2000*GHZ]
    applications = AR
    #workload = 34532
    workload = 10435
    TaskId = 0
    ...
    for comp_source in Comp_source:
        ...
        Latency_comp[comp_source].append(Latency_network)


    x_r = [i/10 for i in range(1,40,1)]
    MakerColor = ["r","g","b","m","g","b","m"]
    MakerShape = ["*","^","s","o","o","s","^"]
    LineShape = ["-","-.","-","-.","-","-.","-"]
    MarkEvery = 20
    for i in range(len(Comp_source)):
        for j in range(len(V2X)):
            n = Comp_source[i]
            comp_n =  Latency_comp[n][0]
            v2x = V2X[j]
            comp_d = comp_n[v2x]
            MakerSC = MakerShape[i]+MakerColor[j]+LineShape[i]
            plt.plot(x_r,comp_d,MakerSC ,markevery=2)

    font2 = {'family': 'Times New Roman',
             'weight': 'normal',
             'size': 6} # 字体大小，相当于也是leged框的大小
    Comp_source = [500 * GHZ, 1000 * GHZ, 1500 * GHZ, 2000 * GHZ, 5000 * GHZ, 10000 * GHZ, 20000 * GHZ]
    plt.legend(["mmwave: 500 ","DSRC: 500", "C-V2I: 500",
                "mmwave: 1000","DSRC: 1000","C-V2I: 1000 ",
                "mmwave: 1500","DSRC: 1500","C-V2I: 1500",
                "mmwave: 2000","DSRC: 2000","C-V2I: 2000",
                "mmwave: 5000","DSRC: 5000","C-V2I: 5000",
                "mmwave: 10000","DSRC: 10000","C-V2I: 10000",
                "mmwave: 20000","DSRC: 20000","C-V2I: 20000"],
