...
def NetServe(v2x,v2xid,R,B,comm_r,gamm=None):
    R = R.copy()
    B = B.copy()
    R[TaskId,v2xid] = 0
    B[TaskId,v2xid] = 0    # 这里的仿真是全部由一个通信来传输，所以没有竞争
    if v2x == "DSRC":
        Netmode, latency = net_mode(v2x, comm_r,gamm=None)
        xi_comm = max(Netmode - sum(R[:, v2x_id[v2x]]),0)
        eta_comm = sum(B[:,v2x_id[v2x]]) + latency # 加一个延迟
        return xi_comm, eta_comm
    elif v2x =="C-V2I":
        xi_comm = net_mode(v2x,comm_r,gamm=None)
        eta_comm = 0
        return xi_comm,eta_comm
    else:
        Netmode = net_mode(v2x, comm_r,gamm)
        xi_comm = max(Netmode - sum(R[:, v2x_id[v2x]]),0)
...
if __name__ == '__main__':
    ...
    #Vehicle_N = [5,10,20]
    vehicle = 5
    CV2I_r = [10,50,100,150,200,250,300]
    for cv2i_r in CV2I_r:
        Latency_comm[cv2i_r] = []
        ...
        for r in range(1,40,1):
            ...
            for v2x in V2X:
                ...
                xi_comm,eta_comm = NetServe(v2x, v2x_id[v2x],R*propotion,B*propotion,cv2i_r,gamm=gamm)
        ...
        Latency_comm[cv2i_r].append(Latency_network)
