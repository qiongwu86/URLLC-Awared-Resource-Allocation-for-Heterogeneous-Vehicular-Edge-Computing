...
class MEC_v1():
    def __init__(self, task_rate, edge_capability,V,*applications,time_delta=10*MS,empty_reward=True,fail_epsilon=0.01):
        super().__init__()
    ...
    def reset(self, empty_reward=True):
        ...
        V = self.V
        self.__del__()
        self.__init__(task_rate, edge_capability,V,*applications)
    ...
    def step(self,actor,memory,Model):
        ...
        if Model=="DRL":
            ...
            #action = np.array([[0.4,0.3,0.25,0]])
            action_comm = np.ones([3,3])*(1/3)
            #action_comm = np.array([[0.2,0.3,0.5],[0.2,0.3,0.5],[0.2,0.3,0.5]])
        elif Model=="Random":
            a = np.random.rand(1,4)
            action = a/sum(sum(a))
            action_comm = np.ones([3,3])*(1/3)

        #print("CPU action : ", action[0][0:3])
        ...
        cost,utility_queue,utility= self.get_cost(state1,action,action_comm,self.V,used_edge_cpus,delay,q0,bt)
        self.timestamp +=1
        return state2,cost,utility_queue,utility,delay,action_comm,action
    ...
    def get_status(self,MasState):
        ...
        for app_type, queue in self.queue_list.items(): # self.queue_list：{"1": TaskQueue, "2":TaskQueue,"3":TaskQueue }
            ...
            if MasState == None:
                ...
                left_mmwave_comp[app_type-1] = self.computational_capability/len(self.queue_list)/GHZ
                left_dsrc_comp[app_type-1] = self.computational_capability/len(self.queue_list)/GHZ
                left_v2i_comp[app_type-1] = self.computational_capability/len(self.queue_list)/GHZ
    ...
    def get_cost(self,state,action,action_comm,V,used_edge_cpus,delay,q0,bt): #used_edge_cpus, used_cloud_cpus, q0, q3, failed_to_offload, failed_to_generate
        ...
        CV2I_COST = CV2I_COST/1e2
        action = action[0][:3]


        qt = np.abs(np.array(q0))
        bt = np.abs(bt)
        #Edge_drift_cost =(sum((qt / MB) * ((arrivals - bt) / MB)) + 1 / 2 * sum(((arrivals - bt) / MB) ** 2)) / 1e3
        # Edge_drift_cost = (sum((qt/MB) * ((-bt/MB))))
        Edge_drift_cost = (sum((qt/MB) * (((arrivals-bt)/MB))))/1e3

        # print(   qt / MB)
        # print((-bt) / MB)
        # print((qt / MB) * ((-bt) / MB))
        # print(sum((qt / MB) * ((-bt) / MB)))

        #print(Edge_drift_cost)
        #print("arrivals : ",arrivals, 'bt : ', bt)
        Computation_cost = compute_cost_fct(10, self.computational_capability*sum(action))/1e5
        #CV2I_COST = 0
        Cost = CV2I_COST + Computation_cost
        Task_require = [30, 30, 30, 30, 30, 30, 30, 30]
        Latency_constraint = 0
        for i in this_app_type_list:
            Latency = max(delay[i]["mmwave"][0],delay[i]["DSRC"][0],delay[i]["C-V2I"][0])
            Latency_constraint += ((Latency-Task_require[i-1])>0)*(Latency-Task_require[i-1])


        reward = (Cost + V * Edge_drift_cost + 1e6*Latency_constraint)/1e5
        print(reward,Cost,CV2I_COST,Computation_cost,V * Edge_drift_cost,1e6*Latency_constraint)
        # print("cost : ",Cost," Edge_drift_cost : ", Edge_drift_cost, " Latency_constraint : ", Latency_constraint, " V : ", V)
        return reward,Cost + V * Edge_drift_cost,Cost