...
from nn_function.eval_util import create_stats_ordered_dict
import nn_function.pytorch_util as ptu
import numpy as np
...
class TanhNormal(Distribution):
    ...
    def log_prob(self, value, pre_tanh_value=None):
        if pre_tanh_value is None:          # 不是很懂
    ...
    def rsample_with_pretanh(self):
        ...
        z = (
                ...
                self.normal_std *
                MultivariateDiagonalNormal(
                    ptu.zeros(self.normal_mean.size()),     # 噪声 ：均值 = 0 ，方差 = 1
                    ptu.ones(self.normal_std.size())
                ).sample()
        )
        return torch.tanh(z), z         # action = torch.tanh(z)
    ...
    def sample_and_logprob(self):
        value, pre_tanh_value = self.rsample_with_pretanh()
        value, pre_tanh_value = value.detach(), pre_tanh_value.detach() # 采样时候不用带梯度
        log_p = self.log_prob(value, pre_tanh_value)
        return value, log_p

    def rsample_and_logprob(self):
        value, pre_tanh_value = self.rsample_with_pretanh()         # value =torch.tanh(z) = action, pre_tanh_value = z
        log_p = self.log_prob(value, pre_tanh_value)
        return value, log_p
